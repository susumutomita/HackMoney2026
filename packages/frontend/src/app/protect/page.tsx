      ...
      <main className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-4 mb-8">
          {[1, 2, 3].map((s) => (
            <div
              key={s}
              className={`flex items-center gap-2 ${step >= s ? "text-cyan-400" : "text-slate-600"}`}
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center border ${step >= s ? "border-cyan-400 bg-cyan-400/10" : "border-slate-600"}`}
              >
                {step > s ? "✓" : s}
              </div>
              <span className="text-sm">
                {s === 1 ? "Connect" : s === 2 ? "Configure" : "Done"}
              </span>
            </div>
          ))}
        </div>
            ...
            {!isConnected ? (
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-slate-500">
                  Enter the Safe you want to protect, or use your connected wallet as owner
                </p>
                <button
                  onClick={() => setStep(2)}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="w-5 h-5"
                />
                <label htmlFor="allowCalls" className="text-slate-300">
                  Allow contract calls
                </label>
              </div>
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4">
                <p className="text-amber-400 text-sm">
                  ⚠️ This will require a transaction to set the Guard on your Safe
                </p>
              </div>
              <button
          ...
          <div className="bg-slate-900/50 border border-emerald-500/20 rounded-xl p-6 text-center">
            ...
            <div className="bg-slate-800 rounded-lg p-4 text-left mb-4">
              ...
              <p className="text-white">
                Max: {maxValue} ETH | Daily: {dailyLimit} ETH
              </p>
            </div>
